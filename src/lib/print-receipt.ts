  ...
  const itemRows = receipt.items
    .map(
      (item, index) => `<tr>
        ...
        <td>${escapeHtml(item.product_name)}<br/><span class="muted">${escapeHtml(item.barcode)} / SN:${escapeHtml(item.serial_number)}</span></td>
        <td class="num">${escapeHtml(item.condition)}</td>
